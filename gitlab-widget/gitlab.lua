...
local GET_PRS_CMD = [[sh -c "curl -s --connect-timeout 5 --show-error --header \"PRIVATE-TOKEN: \"$(pass code.zeekit.co/pat)\"\"]]
...
local function worker(user_args)
	...
	args.host = "https://code.zeekit.co"
	...
	local update_widget = function(widget, stdout, stderr, _, _)
		...
		for _, pr in ipairs(result) do
			...
			row:get_children_by_id("title")[1]:add_button(awful.util.table.join(awful.button({}, 1, function()
	...
	gitlab_widget:add_button(awful.util.table.join(awful.button({}, 1, function()
	...
	watch(
		string.format(GET_PRS_CMD, host),
...
return setmetatable(gitlab_widget, {
	__call = function(_, ...)
		return worker({ host = "https://code.zeekit.co" })
	end,
})
